...
class ModelTestCaseTeste(TestCase):
    """This class defines the test suite for the teste model."""

    def setUp(self):
        """Define the test client and other test variables."""
        self.teste_nome = "Write world class code"
        self.teste = Teste(name=self.teste_nome)

    def test_model_can_create_a_teste(self):
        """Test the teste model can create a teste."""
        old_count = Teste.objects.count()
        self.teste.save()
        new_count = Teste.objects.count()
        self.assertNotEqual(old_count, new_count)
...
class ViewTestCaseTeste(TestCase):
    """Test suite for the api views."""

    def setUp(self):
        """Define the test client and other test variables."""
        self.client = APIClient()
        self.teste_data = {'nome': 'Go to Ibiza'}
        self.response = self.client.post(
            reverse('create'),
            self.teste_data,
            format="json")

    def test_api_can_create_a_teste(self):
        """Test the api has bucket creation capability."""
        self.assertEqual(self.response.status_code, status.HTTP_201_CREATED)

    def test_api_can_get_a_teste(self):
        """Test the api can get a given bucketlist."""
        teste = Teste.objects.get()
        response = self.client.get(
            reverse('details',
            kwargs={'pk': teste.id}), format="json")
